...
public class Swimmer : MonoBehaviour
{
    ...
    public Color drowningColor = Color.red;
    ...
    void Update()
    {
        ...
        {
            // Count down to sinking while drowning
            drownTimer -= Time.deltaTime;
            if (drownTimer <= 0f)
            {
                StartSinking();
    ...
    void TryStartDrowning()
    {
        ...
        {
            isDrowning = true;
            drownTimer = Random.Range(5f, 10f); // time before sinking
            speed = minSpeed * 0.1f;
            spriteRenderer.color = drowningColor;
        }
        else
        {
            drownTimer = Random.Range(minTimeUntilDrown, maxTimeUntilDrown);
    ...
    void StartSinking()
    {
        ...
        // Trigger panic in a random other swimmer
        ...
        if (candidates.Length > 0 && Random.value < panicChance)
        {
            ...
            victim.drownTimer = Random.Range(3f, 8f); // panic shortens their timer
        }
    }
